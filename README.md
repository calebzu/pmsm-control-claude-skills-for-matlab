...
| `.claude/skills/` | Six PMSM modeling skills plus a general-purpose Simulink layout utility (Claude Code skill format), registered automatically when Claude Code opens this folder. |
...
| [`extended/`](extended/) | An extra reference generated by Claude Code following the workflow — a FOC speed controller with a load-torque estimator (Luenberger + Kalman family) + feed-forward compensation. Builds on the skills but is not itself a formal skill. |
