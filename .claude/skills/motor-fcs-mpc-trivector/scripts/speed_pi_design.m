function pidg = speed_pi_design(J, Pn, psif, Tsc, varargin)
%SPEED_PI_DESIGN  Symmetric-Optimum speed-PI gains for a three-vector FCS-MPC loop.
%
%   pidg = speed_pi_design(J, Pn, psif, Tsc) returns a struct with Kp, Ki and the
%   intermediate quantities, using the Symmetric Optimum (SO) tuning for a B=0
%   plant G(s) = Kt/(J*s*(T_eq*s+1)) where the inner three-vector (dual-axis
%   deadbeat) current loop is modelled as a small lag T_eq ~ a few Tsc
%   (near-deadbeat-fast).
%
%   Name/value options:
...
%
%   The three-vector inner loop nulls BOTH id and iq (dual-axis deadbeat), so the
%   torque-producing iq tracks even faster than dual-vector; T_eq=5*Tsc stays a
%   conservative outer-loop lag. (Same machine + Tsc as the dual-vector reference
%   gives identical gains -- expected, not a coincidence.)
%
%   Example (paper SPMSM Table 3, Xu Yanping 2018):
%     speed_pi_design(0.0012, 4, 0.24, 100e-6)
...
pidg = struct('Kt', Kt, 'T_eq', T_eq, 'a', a, ...
              'Kp', Kp, 'Ki', Ki, ...
