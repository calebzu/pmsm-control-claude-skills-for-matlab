function build_foc_ltid_run1()
% BUILD_FOC_LTID_RUN1  Phase-8 generalization-verification copy of build_template.m.
% Topology matches ../scripts/build_template.m;
% ONLY the InitFcn PARAMETER SURFACE + scenario + model name + root-path were retargeted for a
% NEW machine (Rs=1.2, Ld=Lq=8.5e-3, psif=0.175, pn=3, J=3e-4, B=8e-5) under a const-speed
% (80 rad/s) + sustained load-step (+1.5 N.m @ 0.2s) scenario. EQN plant (plant_sel=1).
%
...
initfcn = strjoin({ ...
 ...
 'Rs=1.2; Ld=8.5e-3; Lq=8.5e-3; psif=0.175; pn=3; J=3e-4; B=8e-5;'
 ...
 '% ===== outer speed PI (Symmetric Optimum; see Rule 8) + back-calc anti-windup ='
 ...
 '% ===== scenario ====='
 'wref_val=80; t_ramp=0.01;             % const speed reference (rad/s mech), one-shot ramp'
 'tL_on=0.2; tL_off=0.40; TL_val=1.5; StopTime=0.40;  % load STEP at tL_on, held to StopTime (tL_off unused)'
...
set_param([mdl '/TL_src'], ...
    'VariableName','[0 0; tL_on 0; tL_on TL_val; StopTime TL_val]', ...
    'Interpolate','on','SampleTime','0','OutputAfterFinalValue','Holding final value');
...
for ff=[0 1]
    ...
    win = (tt>=0.20 & tt<=0.30);             % load-step transient window
    wref_hi = sp(end,1);
